...
def build_telegraph_nodes_from_text(text: str, title: str) -> list:
    ...
    for url in imgs[:10]:  # cap to avoid overly long pages
        nodes.append({"tag": "figure", "children": [{"tag": "img", "attrs": {"src": url}}]})
    # add paragraphs for non-empty lines (strip markdown headers)
    for line in text.splitlines():
        l = line.strip()
        if not l:
            continue
...
async def calendar_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not text_result:
        ...
        if page_url:
            ...
            await safe_reply_text(update, msg, disable_web_page_preview=True)
        else:
...
async def process_user_today(application: Application, user_id, token, semaphore):
    async with semaphore:
        try:
            ...
            if page_url:
                msg = f"📄 今日推荐（图文版）：{page_url}\n\n{summary}"
                await safe_bot_send_message(application.bot, user_id, msg, disable_web_page_preview=True)
            else:
